...
def obtener_recursos_combinados():
    ...
    with Session(engine) as session:
        ...
        resultado = []
        for v in videos:
            resultado.append({"id": v.id, "nombre": v.nombre, "url": v.url, "categoria": v.categoria, "tipo": "video"})
        for p in pdfs:
            resultado.append({"id": p.id, "nombre": p.nombre, "url": p.url, "categoria": p.categoria, "tipo": "pdf"})
        return resultado
